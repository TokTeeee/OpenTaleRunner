...
**7 层 Prompt 分层架构（GM 语义脚手架）**：Prompt 由世界层、角色层、场景层、上下文层、任务指令层、JSON Schema 层和查询协议层逐层叠加组成。每层职责清晰，可独立裁剪和定制，确保 GM 拥有完成叙事所需的最小且最精准的上下文。

> 实际实现: `PromptBuilder` 有 11 个 `build*` 方法（4 private + 7 public），与"七层"无强制一一对应（部分层共用一个方法，部分方法横跨多层）。"七层"是 GM 视角的语义分类，不是代码实现边界。详细方法清单见 §2.1。
